<?php 
	...
	{	
		...
		/*** FYI - The following group of constants generally determines if, how, and to what extent the YouTube API is used! ***/
		...
		const _DB_IPS_TABLE = 'ips';  // Database table that stores IPs used for IP rotation, as per _ENABLE_IP_ROTATION_FOR_VIDEOS above. Use the online tool (https://puredevlabs.cc/build-sql/) or see "ips.sql" in the "/docs" folder for the SQL required to build this table.
		const _DB_IPS_TABLE2 = 'ips_search';  // Database table that stores IPs used for IP rotation, as per _ENABLE_IP_ROTATION_FOR_SEARCH above. Use the online tool (https://puredevlabs.cc/build-sql/) or see "ips_search.sql" in the "/docs" folder for the SQL required to build this table.
		...
		const _ENABLE_TOR_PROXY = false;  // Enable the use of Tor for IP rotation (i.e., the TOR METHOD) instead of the default DATABASE METHOD. (See http://bit.ly/4eX8HpF for Tor setup instructions.)
		...
		const _ENABLE_DYNAMIC_IPV6 = false;  // Enable the automatic generation and use of a different, random IPv6 interface every X duration (denoted by the _DYNAMIC_IPV6_FREQUENCY constant value, below) to circumvent temporary YouTube IP bans. When enabled, a new cron job MUST be scheduled to run "inc/scheduler.php" every minute using the following EXAMPLE command: "* * * * * /usr/bin/php /www/wwwroot/yoursite.com/inc/scheduler.php". (IMPORTANT: Do NOT include the double quotes, and you MUST update directory paths to the PHP binary and scheduler.php in this command, as needed, for your server! To reiterate, this is just an example command.) Also, "inc/scheduler.php" MUST be set to CHMOD 0755 permissions.
		...
		// Enable Proxy use with YouTube URL extraction requests
		...
		// _HTTP_PROXY = "http://username:password@proxyhost.com:7000"
		// _HTTP_PROXY = "http://username:password@192.168.16.1:10"
		const _ENABLE_PROXY_SUPPORT = true;
		const _HTTP_PROXY = "YOUR_PROXY_URL";
